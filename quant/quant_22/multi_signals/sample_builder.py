...
import types
import functools
from util import Stream, Logger, date2str, str2date, now, get_minute_stat_features, getMinuteStatFeatureThread
...
class SampleBuilder:
    ...
    def get_stock_list(self, date):
        ...
        def filter_name(name):
            return ('ST' not in name) and ('*' not in name)  # and ('tui' not in name)
    ...
    def join_price(self,
        ...
        for dt in dt_list:
            if verbose:
                self.logger.info('append date: %i' % dt)
            price_df['T%i' % dt] = price_df['T'].map(lambda s: self.dplus(s, -dt))

        for dt in dt_list:
            if verbose:
                self.logger.info("join %s: date = %s" % (field, dt))
        ...
        self.logger.info('finish joining %s: dt_list = %s, time elapsed: %.2fs' % (field, dt_list, time_elapsed))
    ...
    def join_fundamental(self, verbose=True):
        ...
        start_time = time.time()
        self.logger.info("joining fundamental data ...")
        ...
        for ds in date_list:
            if verbose:
                self.logger.info('joining fundamental: date = %s' % ds)
        ...
        time_elapsed = time.time() - start_time
        self.logger.info("finish joining fundamental data, time elapsed: %.2fs" % time_elapsed)
    ...
    def join_index(self,
        ...
        for dt in dt_list:
            if verbose:
                self.logger.info("join index %s: date = %s" % (field, dt))
            price = price_df[['T%i' % dt, field]]
            price.columns = ['date', output_format % dt]
            self.df = self.df.merge(price, how='left', on=['date'])

        time_elapsed = time.time() - start_time
        self.logger.info('finish joining index %s: dt_list = %s, time elapsed: %.2fs' % (field, dt_list, time_elapsed))

        return self

    # ========================== Minutes Sample Functions ======================= #
